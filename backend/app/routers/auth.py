...
async def google_mobile_callback(code: str = None, error: str = None):
    if error or not code:
        return RedirectResponse("wealthdash://auth?error=auth_failed")
    ...
    if not token_resp.is_success:
        return RedirectResponse("wealthdash://auth?error=token_exchange_failed")
    ...
    if not userinfo_resp.is_success:
        return RedirectResponse("wealthdash://auth?error=userinfo_failed")
    ...
    if email not in ALLOWED_EMAILS:
        return RedirectResponse("wealthdash://auth?error=access_denied")

    session_token = serializer.dumps({"email": email, "name": userinfo.get("name", "")})
    return RedirectResponse(f"wealthdash://auth?token={urllib.parse.quote(session_token, safe='')}")
