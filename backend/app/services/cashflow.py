...
async def monthly_cashflow(uid: str, cutoff: datetime) -> dict:
...
async def monthly_cashflow_cached(uid: str, cutoff: datetime) -> dict:
    ...
    when the blob is missing or stale.
    ...
    data = await monthly_cashflow(uid, cutoff)
