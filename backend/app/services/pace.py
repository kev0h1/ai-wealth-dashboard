    ...
    if include_series:
        ...
        for i in range(total_days):
            ...
            series.append({
                ...
                "sustainable_line":        round(period_allowance * (i + 1) / total_days, 2),
            })
...
def _compute_notable_day(
    ...
    # Classify 70-day history with the same exclusion rules.
    ...
    # Seed every day from the earliest observed txn date so zero-spend days count.
    d = earliest_txn_date
    while d <= today:
        daily_totals[d] = 0.0
        d += timedelta(days=1)
    ...
    # Build weekday baseline EXCLUDING days in the current period.
    weekday_samples: dict[int, list[float]] = {wd: [] for wd in range(7)}
    for day, total in daily_totals.items():
        if day < period_start:   # exclude current period from baseline
    ...
    for day in sorted(d for d in daily_totals if period_start <= d <= today):
        ...
        if median is None:
            continue   # fewer than 6 historical samples for this weekday
        if median <= 0:
            continue
    ...
    # Return the most recent qualifying day.
    return qualifying[-1]
