...
#    loop. This proves the retry actually engages and still lands correctly
#    when a delta op from Settings interleaves between Penny's read and its
#    conditional write (forcing the first CAS attempt to lose the version
#    race and retry), rather than silently dropping Penny's change or
#    raising an error.
# ═════════════════════════════════════════════════════════════════════════
...
def test_cas_full_list_set_retries_past_a_concurrent_delta_and_still_lands(monkeypatch):
    ...
    final = asyncio.run(fake_prefs.find_one({"user_id": UID}))
    # Penny's write is not silently dropped and does not error out despite
    # the mid-race collision -- it retried and landed exactly on its own
    # declared target (a full-list-set is allowed to supersede a delta that
    # raced it -- that is its documented, intentional semantics, see
    # _cas_set_cover_plan_excluded_accounts's own docstring -- the property
    # under test here is that the WRITE ITSELF is never corrupted, dropped,
    # or silently ignored by the race).
    assert final["cover_plan_excluded_accounts"] == ["acc-penny-target"]
