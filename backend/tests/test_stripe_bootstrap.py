...
class _FakePrice:
    def __init__(self, id, lookup_key, unit_amount, currency, recurring, product, metadata, active=True):
        self.id = id
...
def _make_fake_stripe():
    ...
    class _Price:
        ...
        @staticmethod
        def list(**kwargs):
            active_only = kwargs.get("active", None)
            data = [p for p in prices if (p.active if active_only else True)]
