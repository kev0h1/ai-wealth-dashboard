...
// FuelSavingsCard/GroceryBasketCard inside Spend's Transport/Groceries
// CategorySheet). PreferencesContext fetches /preferences exactly once per
// full page load and never refreshes on client-side navigation, so a hook
// instance that only read from rawPrefs would miss a pin toggled by a
// *different* instance until a hard reload — e.g. tapping "Pin to Home" on
// GroceryBasketCard's own CategorySheet instance wouldn't show up on Home.
// Routing every toggle() through this shared store means all mounted
// instances, including ones on other routes, update in lockstep without
