...
const assistants = ["All assistants", "ChatGPT", "Claude", "No-result fixture"] as const;
...
function ActivityRows({ rows, variant }: { rows: Row[]; variant: Variant }) {
  ...
  return <div className={variant === "b" ? "grid gap-4 lg:grid-cols-2" : "space-y-5"}>{[...new Set(rows.map((row) => row.day))].map((day) => <section key={day}><h2 className="mb-2 text-sm font-bold">{day}</h2><div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">{rows.filter((row) => row.day === day).map((row, index) => <div key={row.time} className={`flex min-h-16 items-center gap-3 px-4 ${index ? "border-t border-slate-100 dark:border-slate-700" : ""}`}><Check size={16} className="text-emerald-500" aria-hidden="true" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{row.request}</span><span className="block text-xs text-slate-600 dark:text-slate-400">{row.assistant} · {row.time}</span></span></div>)}</div></section>)}</div>;
}
