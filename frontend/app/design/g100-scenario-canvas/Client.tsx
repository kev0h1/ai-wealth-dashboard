...
export default function Client() {
  ...
  if (state === "empty") return <><EmptyScenario variant={variant} /><FixtureBottomNav /><Controls variant={variant} state={state} dark={dark} /></>;
  const current = model[state];
  const tight = state === "tight";
  const canvas = <section className="glass-hero max-w-[65ch] rounded-3xl p-5"><h1 className="text-balance text-3xl font-bold">What if you add £180 a month?</h1><p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{current.reading}</p><p className="mt-4 border-y border-slate-300 py-3 font-mono text-[15px] font-semibold tabular-nums dark:border-slate-700">Live cash £742 <span className="mx-2 text-slate-400">→</span> <span className={tight ? "text-red-700 dark:text-red-300" : ""}>Possible month end {current.result}</span></p></section>;
  const comparisonRail = <section className="glass-hero max-w-[65ch] rounded-3xl p-5"><h1 className="text-balance text-3xl font-bold">What changes if you add £180 a month?</h1><p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">Compare today&apos;s live cash with a possible month-end outcome before deciding.</p><dl className="mt-5 grid gap-3 sm:grid-cols-2"><div className="border-t border-slate-300 pt-3 dark:border-slate-700"><dt className="text-xs text-slate-600 dark:text-slate-400">Live cash today</dt><dd className="mt-1 font-mono text-xl font-bold tabular-nums">£742</dd></div><div className="border-t border-slate-300 pt-3 dark:border-slate-700"><dt className="text-xs text-slate-600 dark:text-slate-400">Possible month end</dt><dd className={`mt-1 font-mono text-xl font-bold tabular-nums ${tight ? "text-red-700 dark:text-red-300" : ""}`}>{current.result}</dd></div></dl></section>;
  const evidence = <div className="space-y-5"><ScenarioInputs /><Calculation opening={current.opening} result={current.result} tight={tight} /><Card title="Decide later, with this comparison"><p className="mt-2 text-sm text-slate-600 dark:text-slate-400">Live cash remains £742. This scenario is only a comparison.</p><div className="mt-3 flex flex-wrap gap-2"><a href="#calculation" className="inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:text-indigo-300">See the calculation <ChevronRight size={15} /></a><a href="/planning" className="inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:text-indigo-300">Return to Planning <ChevronRight size={15} /></a></div></Card></div>;
  return <main className="min-h-dvh bg-[#f0f2f7] pb-56 text-slate-900 dark:bg-[#0f172a] dark:text-slate-100 lg:pb-24" style={{ "--design-controls-clearance": "108px" } as React.CSSProperties}><div className="mx-auto max-w-6xl px-4 py-8"><a href="/design" className="inline-flex min-h-11 items-center gap-1 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"><ArrowLeft size={16} />Back to design</a><p className="mt-4 text-xs text-slate-600 dark:text-slate-400">Fixture preview only. No balance or plan changes here.</p>{variant === "c" ? <div className="mx-auto mt-8 max-w-3xl space-y-9">{canvas}<div className="flex items-center gap-2 text-sm font-semibold text-slate-600 dark:text-slate-300"><MoveRight size={17} aria-hidden="true" />Trace the possible change, then compare it with today.</div><CalculationPath opening={current.opening} result={current.result} tight={tight} /><div className="grid gap-5 sm:grid-cols-2"><ScenarioInputs /><Card title="Before you decide"><p className="mt-2 text-sm text-slate-600 dark:text-slate-400">The live balance remains £742. This path only makes the possible outcome visible.</p></Card></div></div> : variant === "b" ? <div className="mt-8 grid gap-8 lg:grid-cols-[.75fr_1.25fr]">{comparisonRail}<div id="calculation">{evidence}</div></div> : <div className="mx-auto mt-8 max-w-2xl space-y-8">{canvas}<div id="calculation">{evidence}</div></div>}<p className="mt-10 flex gap-2 text-xs text-slate-600 dark:text-slate-400"><Info size={15} aria-hidden="true" />The forecast is not your live balance.</p></div><FixtureBottomNav /><Controls variant={variant} state={state} dark={dark} /></main>;
}
