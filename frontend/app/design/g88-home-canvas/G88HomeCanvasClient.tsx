...
export default function G88HomeCanvasClient() {
  ...
  return <div className={`${mode === "dark" ? "dark" : ""} min-h-dvh bg-[#f0f2f7] text-slate-900 selection:bg-indigo-200 dark:bg-[#0f172a] dark:text-slate-100`} style={{ colorScheme: mode }}><a href="#content" className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[100] focus:rounded-xl focus:bg-slate-950 focus:px-4 focus:py-3 focus:text-white">Skip to preview</a><PreviewControls variant={variant} state={state} mode={mode} query={query} /><main id="content" className="mx-auto max-w-6xl px-4 pb-40 pt-6 sm:px-6 sm:pt-10 lg:pb-16"><header className="mb-8"><a href="/design" className="inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:text-slate-400"><ArrowLeft size={16} aria-hidden="true" />Design rounds</a><h1 className="mt-4 text-balance text-[30px] font-bold tracking-[-.04em] text-slate-950 dark:text-white">Home, on the canvas</h1><p className="mt-2 max-w-[65ch] text-sm leading-6 text-slate-600 dark:text-slate-400">{variants[variant].summary} Every card below is a real production component, rendered with local fixtures only.</p></header>{layout}</main><FixtureBottomNav active="Home" /></div>;
}
