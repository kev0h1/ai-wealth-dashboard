...
export default function MirrorCanvasClient() {
  ...
  return <main className={`${dark ? "dark" : ""} min-h-dvh bg-[#f0f2f7] pb-32 text-slate-900 dark:bg-slate-900 dark:text-slate-100`}>
    <div className="mx-auto max-w-4xl px-5 py-8">
      ...
      <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-slate-600 dark:text-slate-300">Built from connected transactions. No judgement, just a reading that grows clearer with time.</p>
      {state === "empty" ? <section className="mt-12 max-w-2xl" aria-labelledby="not-enough-data"><h2 id="not-enough-data" className="text-lg font-bold">Not enough data yet</h2><p className="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300">The Mirror starts after 60 days and becomes more reliable as more months arrive.</p></section> : <>
        <section aria-label="Behavioural reading" className={`mt-10 ${variant === "b" ? "grid gap-8 lg:grid-cols-2" : "max-w-2xl"}`}>
          {traits.filter((_, index) => index === 0 || revealSecond).map(([id, title, body, category, colour]) => <article key={id} className="border-b border-slate-200 pb-7 dark:border-slate-700">
            ...
            {open === id && <div id={`evidence-${id}`} className="mt-2 rounded-2xl border border-slate-200 bg-white p-4 text-sm leading-relaxed text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">Three categorised payments support this reading. <Link href="/transactions" className="font-semibold text-indigo-700 underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:text-indigo-300">View payments</Link></div>}
            <TraitAction id={id} category={category} choice={choices[id]} onChoice={choose} onSetAim={setAimCategory} />
          </article>)}
        </section>
        {aimCategory && <section className="mt-10 max-w-2xl rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800" aria-labelledby="aim-title"><Target aria-hidden="true" size={18} className="text-indigo-600 dark:text-indigo-400" /><h2 id="aim-title" className="mt-2 font-bold">What you&apos;re working on</h2><p className="mt-1 text-sm leading-relaxed text-slate-600 dark:text-slate-300"><span className="font-mono tabular-nums">£74</span> of your <span className="font-mono tabular-nums">£120</span> {aimCategory} aim · 8 days left</p><p className="mt-3 text-sm font-semibold text-indigo-700 dark:text-indigo-300">Adjust your aim in Planning</p></section>}
      </>}
    </div>
