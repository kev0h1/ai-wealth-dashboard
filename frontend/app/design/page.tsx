...
const ROUTES: PreviewRoute[] = [
  ...
  },
  {
    ...
    description:
      "G124/G127 approved (variant A, cluster interval rule, Kevin 2026-09-18) and folded into production by G131. The hero, the bounded day-card grammar and the cluster-marker algorithm are no longer reimplemented here — this preview imports the SAME shared components and functions PlanningPage.tsx now renders (components/upcoming/UpcomingHeroCard.tsx, components/upcoming/UpcomingDayCard.tsx, components/upcoming/UpcomingDivider.tsx, lib/upcomingMarkers.ts), so those three pieces can't drift from what shipped. The \"gap\" and \"rhythm\" interval rules and the switcher that used to compare all three are gone; only cluster ships. Settling-row correction (G131, reversing an earlier misreading): the long \"Left earlier today, still settling\" line under the payment name is gone, and the right-hand slot under the figure reads \"Settling\" (capitalised) where a live row's \"After: £X left\" sits; the clock icon chip is unchanged. Header typography matches the codex reference: no \"UPCOMING\" eyebrow, h1 at 28px/bold/tight tracking, header row items-start, hero micro-label promoted to a real h2. Day headings carry the absolute date (\"Mon 21 Sep\"), keeping the Today/Tomorrow word where it applies. Red in the hero is confined to the headline figure and the \"N accounts short\" badge only. STILL FIXTURE-ONLY, disclosed as such: the individual ROW (DayGroups.tsx's `Row`) is a hand-authored match against representative fixtures, not an import — PlanningPage.tsx's `renderRow` is a page-scoped closure over live risk-walk state, edit sheets and dismiss handlers that is out of this fold-in's scope to extract. The Set Aside section (variant A/B/C) is UNCHANGED and still undecided, not part of the G131 fold-in. No API calls, no production edits beyond the shared components above · ?variant=a|b|c&state=positive|negative&mode=light|dark",
