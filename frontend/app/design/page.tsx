...
const ROUTES: PreviewRoute[] = [
  ...
  },
  {
    ...
    description:
      "G124: Kevin reviewed G90 (upcoming-canvas-before-cards) against the LIVE /upcoming page: \"good and clean but it changes too much of the existing infrastructure in place\", so this round is NOT a rebuild. PlanningPage.tsx is untouched; this takes exactly three things from G90/Kevin's notes: (1) the hero adopts G90's bounded-panel styling (rounded-3xl p-5/sm:p-6, text-[40px] figure) with every piece of live CONTENT unchanged; (2) same-day payments group into one bounded day section adopting the G122 transactions-hub grammar verbatim (rounded-2xl bordered section, divide-y hairline rows) in place of a floating glass-card per row, preserving the payday-boundary divider, the settling sub-cluster and data-day-key; (3) three different answers to Kevin's \"Set aside... makes it cluttered\" note (a bare-number title, a shouty truncated raw bank string, a hedge-tilde/minus collision): A tightened ledger (typography/truncation fixed in place), B compact chip (the raw feed identity moves into a small pill), C progressive disclosure (title, amount and a progress bar by default, cadence and the fed-by line open on tap). REVISED 2026-09-18 after Kevin's phone review, applied to all three variants: rows inside a day group no longer repeat the day's own date (the day heading already carries it, following TransactionRow's showDate pattern); a settling row now shows only the clock icon chip and the \"Left earlier today, still settling\" line, the SETTLING section title and the right-hand settling caption are gone, and settling rows still sort to the end of their day group; the hero's red panel tint is removed entirely in every state (the earlier `?redRule=unified|live` toggle, its explanatory note and the HERO_DIVERGENT fixture are gone with it). ROUND THREE (G127): red narrows again, now living only in the headline figure and the \"N accounts short\" badge; the shortfall attribution paragraph, its border and the Review link are neutral, each reusing a colour already established elsewhere on the card rather than a red of their own. The page header adopts the codex reference's typography exactly: the \"UPCOMING\" eyebrow is gone, the heading row aligns items-start (not items-center), and the h1 takes codex's 28px/bold/tight tracking and ink; the hero's own micro-label becomes a real heading to match. Day headings now carry the absolute date instead of a relative day count (\"Mon 21 Sep\" rather than \"3 days\"), keeping the Today/Tomorrow word prefixed onto the date where it applies; the relative sense of time moves onto the canvas instead, as a switchable marker between day groups with three genuinely different rules (a gap of 7+ days, fixed weekly/fortnightly/four-weekly rhythm horizons, or a run of 3+ tightly-spaced groups reading as a clutter), toggled live via the switcher's second row or `?interval=`. Where a rule's marker would land on the exact seam as the payday boundary, the two do not concatenate into one line (that broke the divider's hairlines at 390px in the rejected round two, G127 round three fix): the payday text keeps the hairline/label row, and the marker's own label sits underneath as a quiet caption. FIXTURE-ONLY and disclosed as such: PlanningPage.tsx's hero figure, risk flags and day-group walk are computed inline inside one large authenticated page component with no importable boundary, so this hand-authors markup against representative fixtures rather than rendering PlanningPage.tsx (see fixtures.ts's own header). No API calls, no production edits · ?variant=a|b|c&state=positive|negative&mode=light|dark&interval=gap|rhythm|cluster",
