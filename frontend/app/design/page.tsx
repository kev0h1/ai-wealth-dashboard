...
const ROUTES: PreviewRoute[] = [
  {
    ...
    description:
      "G168, skill: impeccable · variant A folded in 2026-09-26: the month-closed (\"needle\") card, \"Your month closed on Thursday. Here's how it went\", used to have no dismiss on Home and was absent from the Penny hub · Kevin's pick, 'Chip and chevron' — Home keeps the card as it always was plus the standard glass × in its usual top-right slot, wired to a real server dismiss (keyed needle:<period_end>, companion.py already honours that dismissed set); Penny renders the same card permanently in its own section under the payday plan, with a chevron-up Minimise (never a dismiss, mirroring the payday plan's own owner rule, G164) that collapses it to a one-line row, tapping the row expands it back · renders the PRODUCTION component directly (components/MonthClosedCard.tsx), which HomeBrief.tsx's BriefBody and app/penny/PennyPage.tsx both now import, not a standalone reimplementation · states: Home (with its dismiss chip), Penny (expanded, chevron), Penny minimised (collapsed row) · fixture data only, no API calls — the dismiss/minimise controls in this preview are local no-ops / MonthClosedCard's own localStorage collapse, never a server call · ?state=home|penny|penny-minimised&mode=light|dark",
    states: [
      { label: "Home", value: "home" },
      { label: "Penny", value: "penny" },
      { label: "Penny · minimised", value: "penny-minimised" },
    ],
  },
  {
  ...
  },
  {
    ...
    name: "payday-plan-executed",
    description:
      "G164, skill: impeccable · Kevin 2026-09-25: 'If it's already split why am I seeing this and why can't I dismiss it', clarified 08:03 — on Penny the plan always exists and can never be dismissed (today's × already only collapses, HomeBrief FIX D; the affordance just reads wrong), on Home the whole component can be genuinely dismissed · three variants on the executed (already-split) payday plan state across surface=home|penny and state=row|expanded: A 'Receipt' — a past-tense receipt ledger, Penny's header carries a chevron-up Minimise only, Home adds a quiet 'Dismiss' text control in the footer alongside it / B 'Fold' — the row itself is the disclosure header (aria-expanded), no X or separate close control anywhere, Home gets a small dismiss chip beside the row in the app-wide glass '×' treatment, Penny none / C 'Celebration line' — Home drops the expandable card for a one-line dismissible celebration since Upcoming already lists the standing orders (no-app-artefacts rule), Penny keeps the full past-tense card with a minimise · every variant reports what actually moved (£3,170 across 7 standing orders, Fri 25 Sep) rather than the plan's own understated legs (£2,725 to 3) — real total and count, illustrative per-account split: companion.py has no record of which account received how much, so the 7 rows are an invented but honestly-labelled split across Kevin's known destination accounts (all bank-badged, all 'received', never a mix with external bill payees), sized to sum to the real £3,170 · production PaydayPlanCard.tsx/HomeBrief.tsx are read but unmodified by this round; BankBadge, PennyMark's gradient chip and the glass-card/DismissChip visual tokens are reused, the row copy, past-tense ledger and every control are hand-authored since changing exactly those is the point of the round · Kevin's real 25 Sep payload, authorised for this public preview, real total/count/landed-time, illustrative per-row split · fixture data only, no API calls · ?variant=a|b|c&surface=home|penny&state=row|expanded&mode=light|dark",
    states: [{ label: "Everything", value: "everything" }],
    variants: [
      { label: "A · Receipt", value: "a" },
      { label: "B · Fold", value: "b" },
      { label: "C · Celebration line", value: "c" },
    ],
  },
];
