...
function CanvasHeading({ id, title, copy, icon: Icon }: { id?: string; title: string; copy: string; icon?: typeof Target }) {
  return (
    <div className="max-w-[65ch] px-1">
      <div className="flex items-center gap-2">
        {Icon && <Icon size={16} className="text-indigo-600 dark:text-indigo-400" aria-hidden="true" />}
        <h2 id={id} className="text-pretty text-base font-bold text-slate-900 dark:text-slate-100">{title}</h2>
      </div>
...
function Goals({ onOpen }: { onOpen: (goalName: string) => void }) {
  return (
    <section id="commitments" className="scroll-mt-6" aria-labelledby="g89-goals-heading">
      <CanvasHeading id="g89-goals-heading" title="Long-term goals" icon={Target} copy="Each goal keeps its own target, pace and edit decision together." />
      <div className="mt-3 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:shadow-none">
        {GOALS.map((goal) => <GoalRow key={goal.id} goal={goal} hideValues={false} onOpen={() => onOpen(goal.name)} />)}
      </div>
    </section>
  );
}

function Work({ position, onAction }: { position: Position; onAction: (message: string) => void }) {
  const view = growView(position);
  return (
    <div className="space-y-9">
      <section id="priorities" className="scroll-mt-6" aria-labelledby="g89-priorities-heading">
        <CanvasHeading id="g89-priorities-heading" title="Your priority order" icon={Flag} copy="Completed and later steps fold away so the decision that is live now is the part that gets the space." />
      ...
      <section id="debt" className="scroll-mt-6" aria-label="Debt evidence">
        <DebtPosition debt={DEBT_SUMMARY} hideValues={false} onOpen={() => onAction("Debt details would open here in the signed-in product.")} />
      </section>
      <Goals onOpen={(goalName) => onAction(`${goalName} would open for editing in the signed-in product.`)} />
    </div>
  );
}

function VariantA({ position, onAction }: { position: Position; onAction: (message: string) => void }) {
  const view = growView(position);
  return <div className="space-y-8"><GrowHero view={view} hideValues={false} onSeeDue={() => onAction("Upcoming payments would open here in the signed-in product.")} /><div className="border-b border-slate-300/80 pb-6 dark:border-slate-700"><CanvasHeading title="A plan with one job at a time" icon={CircleDollarSign} copy="This month’s position is the instrument. The sections below show the order, debt evidence and goals you can change without wrapping the explanation in extra surfaces." /></div><Work position={position} onAction={onAction} /></div>;
}

function VariantB({ position, onAction }: { position: Position; onAction: (message: string) => void }) {
  const view = growView(position);
  return (
    <PlanningComposition
      view={view}
      hideValues={false}
      onSeeDue={() => onAction("Upcoming payments would open here in the signed-in product.")}
      cashSlot={<CashAndInvestments view={view} hideValues={false} />}
      debtSlot={<section id="debt" className="scroll-mt-6" aria-label="Debt evidence"><DebtPosition debt={DEBT_SUMMARY} hideValues={false} onOpen={() => onAction("Debt details would open here in the signed-in product.")} /></section>}
      goalsSlot={<Goals onOpen={(goalName) => onAction(`${goalName} would open for editing in the signed-in product.`)} />}
    />
  );
}

function VariantC({ position, onAction }: { position: Position; onAction: (message: string) => void }) {
  const view = growView(position);
  return (
    <div className="space-y-8">
      <GrowHero view={view} hideValues={false} onSeeDue={() => onAction("Upcoming payments would open here in the signed-in product.")} />
      ...
      <Work position={position} onAction={onAction} />
    </div>
  );
}
