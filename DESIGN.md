...
A ledger is the arithmetic behind a figure the user has already been given: operand rows, each carrying its sign at the start of the row and its amount in mono tabular figures at the end, closing on one derived total, all behind a quiet indigo disclosure summary. Safe to Spend's "How we got £X", Upcoming's "Full calculation" and Planning's "How we calculated this" are the same object and read in the same grammar. They differ only in how much a row has to say: Safe to Spend gives each row an operator column and an optional wrapped detail caption, while Upcoming and Planning rows are single-line label-and-figure pairs.

**One Separator Per Ledger Boundary (G154, 2026-09-23).** Any two adjacent ledger rows are separated by exactly one device, and the derived total is marked by exactly one device no operand row wears. Which device is a local choice, and the shipped ledgers legitimately differ: Safe to Spend's rows wrap and carry captions, so a hairline between rows earns its place and the total takes that same hairline drawn heavier; Upcoming's and Planning's rows are one line each, so spacing alone separates them and the total takes a rule (Upcoming) or a tinted status row (Planning). The count is not a local choice. Two devices at one boundary read as an accountancy double underline that nobody chose.

Draw row separators on the top edge of every row after the first (`first:`), never on the bottom edge with a `last:` escape. A total is a sibling of the operands, so the last operand is never `:last-child` and the escape never fires; `divide-y` carries the same fault because Tailwind v4 compiles it to `:where(.divide-y > :not(:last-child))`. On the top edge, the total's heavier rule *is* the rule at its own boundary rather than a second line under the one above it.
...
The answer-first summary is plain canvas, not another card. "Your pay period, as it happened" leads, followed by the server-computed reading and a quiet In, Out and Moved separately ledger. Out remains the spending figure, while Moved stays neutral ink and explicitly outside Out. A quiet, collapsed "How £X out adds up" disclosure underneath reconciles the amount needing a look, other categorised spending, anything still to categorise and the total Out. On desktop this summary forms the sticky left rail; the journey runs beside it. Mobile stacks the same information in that order.
...
The journey line then records pay arrival, explains the pace changes, asks for any unresolved placement, shows quieter spending, separates moved money, shows the pay-shape instrument and ends with the user's charts. The pace explanation reconciles positive notable-category excess with the offset across everything else to the overall actual-versus-usual difference. Notable categories keep the existing resolve-in-place lifecycle and use equal two-line One-off and New normal choices, with neither visually recommended over the other.
