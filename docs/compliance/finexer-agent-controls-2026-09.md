...
Drafted 2026-09-06 from the codebase, SECURITY.md, PRIVACY.md and TERMS.md. Items marked [KEVIN] need a decision or a fact only the directors hold. Each answer block must stay under 2000 characters: count only the text inside the fenced ```text``` block under each question, exactly as written, including any open `[KEVIN: ...]` marker (the heading and the `Status:` line are not part of the answer and do not count). Markers are counted rather than excluded because an open marker still needs real replacement text before that answer is submission-ready, and using the marker's own length as a stand-in keeps the block honest about how much room is left; re-measure after resolving any marker. The branch this document was drafted against was deployed to production (Vercel and both Railway services) and tagged `release-20260910-1137` on 2026-09-10; answers that say "production" describe that deployed state. See "Prerequisites" at the end for what is still outstanding.
