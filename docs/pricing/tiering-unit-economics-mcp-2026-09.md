...
1. **Usage metering (first, before any cap).** New `llm_usage` collection: user_id, pipeline, model, input tokens, cached input tokens, output tokens, cost in USD, timestamp, from every OpenRouter response's `usage` object. Monthly rollup per user and per pipeline. This also answers "what does AI cost us" with real numbers instead of this estimate.
   ...
   **Cost dashboard.** `GET /admin/llm-usage?month=YYYY-MM` (bot credential with `admin:usage` scope, or owner session — A28, see docs/ops/ENV.md's "Bot/service credentials" section) reads that same `llm_usage` collection and returns totals, a per-pipeline breakdown, a per-user breakdown with tier and top pipeline, and mean/median/p90 cost per user, for one calendar month at a time. Call it with a bot credential minted via `backend/scripts_bot_credential.py create --name usage-dashboard --scopes admin:usage`: `curl -H "Authorization: Bearer $BOT_TOKEN" "$API/admin/llm-usage?month=2026-09"`. Its per-user cost, call and token figures supersede the estimates in this section, so treat this endpoint as the source of truth for real AI cost per user from here on.
...
### MCP call pack (F9, 2026-09-08)

One pack, not three: `MCP_CALL_PACKS` is a single row, 1,000 calls for £2.99 (2.99p per call, close to the Connect-to-Max marginal rate of 3,000 extra calls for £4, 0.13p each, since the tier jump is priced for volume a pack can't match). Connect is the only tier that would ever reach for this pack today (2,000 calls/month); Max already carries 5,000. Same mechanics as the Penny packs above end to end, sharing one settlement implementation (`app.core.subscription._settle_packs`, parameterised by collection, tier-limit key and a usage-counting callable) rather than a second copy: 90-day life from purchase, drawn down only after the tier's own monthly allowance is spent, no purchase flow yet (billing is B5, so the row on Settings' Connected assistants card reads "Available soon" the same way). `GET /subscription`'s new `mcp` block (`{limit, used, remaining, resets_on, packs, pack_calls}`) and top-level `mcp_packs` serve it; `POST /subscription/admin/topup` grants one via `kind: "mcp"` (bot-only, same as the Penny grant path) until then.
