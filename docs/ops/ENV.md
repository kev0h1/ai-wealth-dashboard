...
| `API_PROXY_SECRET` | `frontend/proxy.ts` | absent | present (production only) | optional; A110, the frontend half of backend's `TRUSTED_PROXY_SECRET` (see that row in the Backend table above), same value on both platforms, generate once with `openssl rand -hex 32`. Server-only: deliberately NOT a `NEXT_PUBLIC_` variable, so it is never bundled into client JavaScript. `frontend/proxy.ts` (matched to `/api/:path*` only) reads it fresh per request and, when set, adds it as the `X-Sorted-Proxy-Auth` request header on the outgoing rewrite to the backend; when unset (UAT, local dev) the proxy adds no header and every request behaves exactly as it did before A110. Never runs at all under `MOBILE_EXPORT` (a static export has no server, hence no proxy/middleware step), which is fine: the mobile apps reach the backend directly and are meant to fall back to plain `TRUSTED_PROXY_HOPS`, not this header. |
