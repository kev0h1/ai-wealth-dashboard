...
| `REDIS_URL` | Two separate instances, rotated independently: production's is the Railway Redis plugin, provisioned for both Railway services (`ai-wealth-dashboard`, `worker`); UAT's is a local, Docker-proxied `redis:7-alpine` container on this VPS. Rotating either means recreating/repointing that instance and updating this URI wherever it's `present` for that environment. | Rate limiting falls back to the in-process, per-replica approximation (A27's territory); the OAuth pending-request store and any other Redis-backed cache fall back to their own in-process equivalents; background job queueing (arq) has no in-process fallback and stops working entirely until every worker has the new URL. |
