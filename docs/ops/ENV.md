...
| `TRUSTED_PROXY_HOPS` | `core/config.py` | absent (default `0` is safe but coarse today; Kevin should set `1`, see note) | absent (default `0` is safe but coarse today; Kevin should confirm and set, likely `2`, see note) | optional (default `0`); A92. The number of trusted reverse-proxy hops in front of this app that themselves append the real client address to `X-Forwarded-For`; `core/ratelimit.py`'s `client_ip()` reads exactly that many entries from the right-hand end of the header instead of trusting a caller-supplied value (`X-Real-IP` is no longer read at all). Default `0` means "trust no forwarded header, use the raw socket peer", the safe fallback: it can only under-differentiate clients sharing one proxy (one shared rate-limit bucket), never let a caller pick its own bucket. **UAT = `1`, confirmed**: this host's nginx site config (`/etc/nginx/sites-enabled/uat.wealth.auriqltd.co.uk`) unconditionally overwrites `X-Real-IP` with `$remote_addr` and sets `X-Forwarded-For` via `$proxy_add_x_forwarded_for`, which appends nginx's own observed address after anything a client sent rather than replacing it, so the rightmost entry is always nginx's own, one hop. **Production: proposed `2`, NOT verified live, Kevin must confirm the exact hop count before setting this at all**: the chain is client → Vercel's `/api/*` rewrite (`frontend/next.config.ts`, see `RELEASE.md`) → Railway's own edge → this app. Vercel and Railway are both commonly documented to append the connecting client's address to `X-Forwarded-For` rather than overwrite it, which would make `2` correct (Vercel's hop, then Railway's hop, both to the right of anything a caller sent), but this session could not find a citable spec confirming either platform's exact behaviour on this project's specific rewrite path, and the earlier A49/WP2 pentest run only proved the *current, unbounded* header is spoofable, not the hop count a bounded version should use. Confirm by temporarily logging the raw `X-Forwarded-For` header on one real, unspoofed production request (e.g. one log line in `client_ip()` behind a flag, removed after) and counting comma-separated entries against a known request. **The two misconfiguration directions are not equally safe, unlike REDIS_URL-style "just imprecise" settings: setting the hop count LOWER than the true number of appending hops is always safe** (it reads an entry an upstream proxy itself wrote, at worst the outer proxy's own address shared by everyone behind it, i.e. a coarser but still caller-proof bucket); **setting it HIGHER than the true number reopens the exact spoofing bypass this fix exists to close**, because it walks past the hops that actually append and reads an entry the caller supplied itself. If in doubt, set it one lower than the count observed, not higher. |
...
|---|---|---|
| `MONGO_URI` | Production only, via the Atlas dashboard: create a new database user (or rotate the existing user's password), update the connection string on both Railway services, then remove the old user once both are confirmed on the new one. UAT's `mongod` has no authentication configured, so there is no credential to rotate there; if that ever changes, rotate it independently, it is a separate database, not the same value. | Every DB-touching request on the service being rotated fails until every process (API, worker) has the new URI and has restarted; do not remove the old Atlas user until both are confirmed rotated, or you can lock yourself out mid-rotation. |
...
| `REDIS_URL` | Two separate instances, rotated independently: production's is the Railway Redis plugin, provisioned for both Railway services (`ai-wealth-dashboard`, `worker`); UAT's is a local, Docker-proxied `redis:7-alpine` container on this VPS, started with `--requirepass`, so UAT's `REDIS_URL` also carries a credential. Rotating either means recreating/repointing that instance and updating this URI wherever it's `present` for that environment; on UAT specifically, that means changing the container's `--requirepass` value and `backend/.env`'s `REDIS_URL` together, then restarting `wealth-api` and `wealth-worker` to pick up the new value. | Rate limiting falls back to the in-process, per-replica approximation (A27's territory); the OAuth pending-request store and any other Redis-backed cache fall back to their own in-process equivalents; background job queueing (arq) has no in-process fallback and stops working entirely until every worker has the new URL. |
