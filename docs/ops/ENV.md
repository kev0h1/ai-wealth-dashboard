...
| `STRIPE_PRICE_IDS` | `core/config.py` | absent | absent | optional; comma-separated `key=price_id` pairs. Each paid tier needs monthly (`lite`, `standard`, `connect`, `max`) plus `_<three_months\|six_months\|annual>` recurring prices; packs use `penny_small`, `penny_medium`, `penny_large`, `mcp_1000`. Statements is free. `BILLING_ENABLED` only goes true once `STRIPE_SECRET_KEY` and all 20 price keys are present, so partial configuration stays "not live". |
