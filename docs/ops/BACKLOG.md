...
- `abandon` deletes the worktree and its local branch and, for a
  not-cancelled item, resets it to to-do with a note, for a session that
  didn't pan out. For a `cancelled` item (H80 correction round) it does
  NOT reset to to-do — the item stays `cancelled`, with a note recording
  the abandon and its now-dangling `[branch: ...]` tag cleared instead
