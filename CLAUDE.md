...
- **Penny**: screen keys now include `upcoming`; `lib/pennyScreenConfig.tsx` has separate `planning` and `upcoming` configs. Tutorial flows split the same way (`TutorialContext.tsx`). Payday plan lifecycle (G164, 2026-09-26): advisory only; Home shows it from five days before payday, dismissible, and nothing after the pay lands; Penny always shows the plan for the next payday, and its live card carries the same minimise chevron as the expanded preview, never an X. A plan whose destinations already clear on their own the moment it would first be proposed is never shown at all (decided before persistence, not reactively afterwards); a plan persisted active that clears in a LATER run also goes quiet rather than celebrating, unless an observed credit into a destination since the doc's `created_at` is NOT attributable to one of the user's own learned recurring standing-order series (`_pp_funded_by_hand`, `companion.py`) — the Sorted celebration is reserved for a plan the user acted on by hand.
- **Backend Safe-to-Spend hardening** (`routers/analytics.py`): Yapily accounts are consent-filtered (`AUTHORIZED` only); same-day events walk debit-before-income; own-account moves whose destination is already in the pool are excluded (`pooled_transfers_excluded`); if any reserve lookup fails the result is `calculation_status: "degraded"` and `safe_to_spend` is clamped to ≤ 0 (fail closed). The Kenya region and its `insufficient_data`/`calculation_status: "unsupported"` branch were removed entirely by A98; `calculation_status` is now just `"complete"` or `"degraded"`. New fields: `lowest_projected_balance`, `allocations_reserved`, `allocations_count`, `unavailable_components`. `PATCH /preferences` wipes the user's whole response cache. `spend_impact.py` reads the cached Safe-to-Spend rather than recomputing, and only computes headroom on the under-usual branch. Tests: `tests/test_safe_to_spend_hardening.py`, `tests/test_spend_performance_guards.py`.
...
`TODO.md` and the Finexer compliance doc are the board: content and workflow (state, owner, notes) live in the same markdown, git is the history, and the private page `/ops/go-live` reads and writes them live. See `docs/ops/BACKLOG.md` for the full model, including the state machine (`todo` / `in-progress` / `blocked` / `review` / `rejected` / `uat` / `cancelled` / done) and the `add`/`review`/`reject`/`uat`/`approve`/`cancel`/`uncancel`/`todo` commands. `rejected` is what a reviewer sets the instant they find a defect in an item sitting in `review`, using `scripts/backlog.py reject <ID> "<reason>"` (or the Reject control on `/ops/go-live`), never by leaving it in `review` while the fix happens elsewhere: `review` alone reads as consent to merge to any integrate pass, including one from a concurrent session, so a rejection that only exists in conversation can and has been overtaken by a pass that merges the very branch just rejected (see docs/ops/BACKLOG.md for the exact 2026-09-10 incident this closes). `start` or `todo` moves an item back out of `rejected`. `uat` is the same shape for a design round (H31): `scripts/session.sh finish <ID> --uat-review` (or integrate's own backstop over the merged diff's paths) lands a clean merge there instead of `done`, with a preview link on `https://uat.wealth.auriqltd.co.uk/design`, a push to Kevin through the existing FCM/APNs path, and a UAT column on the board next to Rejected; `scripts/backlog.py approve <ID> "<choice>"` (or the page's Approve control) records Kevin's pick and reopens the item as `in-progress` with its owner unchanged and no branch recorded, the one `in-progress` shape `scripts/session.sh start` now accepts besides `todo`, so the same agent runs `start <ID>` again and gets a fresh worktree to fold the winner in, see "Design work" above. `cancelled` (H80) is Kevin's own call that a piece of work should not happen at all — obsolete, superseded, or simply not wanted — distinct from `rejected` (defective, fix it) and `blocked` (can't proceed yet): `scripts/backlog.py cancel <ID> "<reason>" --actor kevin` (or the Cancel control on `/ops/go-live`). Be precise about where this is actually enforced, because two prior rounds overclaimed it: `/ops/go-live` is the genuinely kevin-only path, because that page is gated by real account-owner auth end to end and hardcodes the actor to `kevin` regardless of who is signed in — but that page's own `ItemDetailSheet` "Move to" chips (To do, In progress, Done, Blocked) mostly fire straight through on a cancelled item with no refusal at all, since `backend/app/routers/ops.py` calls the service layer directly and never goes through the CLI's guard; this is accepted for To do, In progress and Blocked, since it is still Kevin's own tap behind that same auth, and `set_state` itself still carries no cancelled check, so those three chips (and a `BoardView.tsx` drag onto those columns) still fire straight through unrefused. Done is the one exception, closed as a blocking defect found in the 2026-09-18 review: dragging a cancelled card onto Done used to fire the done action with no confirmation, silently clearing `item.reason` and ticking the item complete, the exact transition the CLI already refused, so the two surfaces disagreed. `TodoDoc.set_done` itself now refuses a cancelled item with no override, the same refusal shape `_refuse_if_cancelled` already used on the CLI, so that one transition is genuinely universal rather than CLI-only, and `backend/app/routers/ops.py` surfaces the refusal as a 4xx with the reason rather than a 500; `BoardView.tsx`'s own `isValidDropTarget` also refuses Done as a drop target for a cancelled card client-side, so the same request is never even sent for a drag. On the CLI, `--actor kevin` is a self-declared flag like every other actor check in this codebase, so `TodoDoc.set_state` refusing any other value is a guard against forgetting, not a barrier against intent — nothing stops an agent typing `--actor kevin` on purpose. (A `BACKLOG_AGENT` environment check was tried as further "defence in depth" and removed: it broke every Codex `finish`, since AGENTS.md has Codex sessions export exactly that variable and `scripts/session.sh finish` runs the whole suite in the session's own environment, and it bought nothing besides — `env -u BACKLOG_AGENT` defeats it in one token.) Either way, an agent should never decide work is unnecessary: leave a note recommending cancellation and let Kevin cancel it himself. Cancelling an already-done item is refused outright, with no override. On the CLI, `start`, `block`, `review`, `reject`, `uat`, `todo` and `done` all refuse a cancelled item outright too, with NO override at all — not a `--force` flag, which would leave a board commit indistinguishable from an ordinary start/todo with no record a cancellation was overridden or why. The only way out is a dedicated verb, `scripts/backlog.py uncancel <ID> "<why>"`, which requires a reason (like `cancel`/`reject` do), writes a dated note, and moves the item to `todo` — kevin-only, the same shape and the same honest framing as `cancel` itself: a cancellation is Kevin's own input, deciding a ticket should not happen, so an agent must not be the one to undo that decision either; an agent that meets a cancelled item leaves a note recommending it be reopened and lets Kevin run `uncancel` himself. This closes the earlier correction-round finding where `scripts/session.sh finish`/`abandon` could silently push a cancelled item into review or back to to-do as a side effect of another command, and the "reject then start" laundering path that needed no flag at all; `scripts/session.sh abandon` on a cancelled item now removes the worktree/branch and clears the item's now-dangling branch tag (refusing to do so on any item that isn't cancelled) without reopening it. A cancelled item is closed but never `[x]` and never counted toward done or outstanding on any progress figure; it is not hidden, but on the phone board it collapses beside To do and Done behind its own count rather than becoming a sixth chip on the counts strip that already overflows a true 390px width. Codex agents follow this same board and branch-per-item workflow; their copy of it lives in `AGENTS.md`, and the same self-declared-actor guard applies there too.
...
backend/.venv/bin/python scripts/backlog.py cancel <item-id> "<reason>" --actor kevin
